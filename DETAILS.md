# RPR1 technical details

This document covers the codec pipeline, container design, algorithm trade-offs, discarded ideas,
and benchmarks. For a project overview and development commands, see [README.md](README.md).
...
#### Looking further ahead than one code

Round-to-round greedy cannot see a pair of codes that only pays *as* a pair — the second half of a
chain nobody has a use for yet. That turns out to be worth nothing, and the reason it cannot be worth
much is worth writing down.

Two codes can only interact through a chain. Everywhere else their gains are additive and so are
their table costs, so if neither pays alone the pair cannot pay either. That confines the search to
codes sitting *back to back* in some word with the existing table finishing the rest — enumerable per
entry, and cheap once pieces are integer ids and entries already costing three bytes or fewer are
skipped: 22,000–34,000 distinct pairs per round, in about 10ms.

Across every round, at most one of those pairs ever beat what greedy takes in two rounds, and only in
the last slots, once the strong single codes are gone. The gain is bounded by the entries *only* the
pair can serve, and that set is always small next to what any decent single code still serves. It is
not that the table saturates — even at 29 codes only 62% of entries can be finished by the table at
all — it is that "pays only as a pair" and "pays a lot" are close to mutually exclusive.

Measured against the objective as it stood before the header codebook, lookahead found one pair
(`war` + `d`) and 30 bytes. Against the objective *with* the codebook it finds `m` + `ent`, wins that
round, and ends 6 bytes worse than plain greedy — a local win that costs more later. The codebook
had already bought what the pair was selling: a cheap header for exactly the short chained entries
these pairs create. Not shipped.
