...
On the full KJV it reaches 1,133,509 bytes with search included, against 958,460 for RPR1 with no
search at all; adding a conventional inverted index to RPR1 costs 501,951 bytes, which puts YLK1
22.4% below the pair. `mise run bench-link` reproduces those numbers and verifies the round trip and
